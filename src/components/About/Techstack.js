import React from "react";
import { Col, Row } from "react-bootstrap";
import { CgCPlusPlus } from "react-icons/cg";
import {
  DiJavascript1,
  DiReact,
  DiNodejs,
  DiMongodb,
  DiPython,
  DiGit,
  DiJava,
  DiCss3,
  DiHtml5,
} from "react-icons/di";
import {
  SiFirebase,
  SiTableau,
  SiVercel,
  SiVisualstudiocode,
} from "react-icons/si";

function Techstack() {
  return (
    <div style={{ paddingBottom: "40px" }}>
      {/* FRONTEND */}
      <h4 style={{ marginTop: "30px", textAlign: "center" }}>Frontend</h4>
      <Row style={{ justifyContent: "center", paddingBottom: "20px" }}>
        <Col xs={4} md={2} className="tech-icons"><DiHtml5 /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiCss3 /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiJavascript1 /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiReact /></Col>
      </Row>

      {/* BACKEND + DATABASE */}
      <h4 style={{ marginTop: "30px", textAlign: "center" }}>Backend & Databases</h4>
      <Row style={{ justifyContent: "center", paddingBottom: "20px" }}>
        <Col xs={4} md={2} className="tech-icons"><DiNodejs /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiJava /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiMongodb /></Col>
        <Col xs={4} md={2} className="tech-icons"><SiFirebase /></Col>
      </Row>

      {/* LANGUAGES */}
      <h4 style={{ marginTop: "30px", textAlign: "center" }}>Languages</h4>
      <Row style={{ justifyContent: "center", paddingBottom: "20px" }}>
        <Col xs={4} md={2} className="tech-icons"><CgCPlusPlus /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiJavascript1 /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiPython /></Col>
        <Col xs={4} md={2} className="tech-icons"><DiJava /></Col>
      </Row>

      {/* TOOLS */}
      <h4 style={{ marginTop: "30px", textAlign: "center" }}>Tools</h4>
      <Row style={{ justifyContent: "center", paddingBottom: "20px" }}>
        <Col xs={4} md={2} className="tech-icons"><DiGit /></Col>
        <Col xs={4} md={2} className="tech-icons"><SiTableau /></Col>
        <Col xs={4} md={2} className="tech-icons"><SiVercel /></Col>
        <Col xs={4} md={2} className="tech-icons"><SiVisualstudiocode /></Col>
      </Row>
    </div>
  );
}

export default Techstack;
